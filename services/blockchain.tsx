...
const payout = async (gameId: number): Promise<void> => {
  console.log('Starting payout function')
  console.log('Input gameId:', gameId)
  ...
  try {
    console.log('Fetching Ethereum contract...')
    const contract = await getEthereumContracts()
    console.log('Contract fetched:', contract)

    console.log('Sending payout transaction for gameId:', gameId)
    const tx = await contract.payout(gameId)
    ...
    console.log('Fetching scores for gameId:', gameId)
    const scores: ScoreStruct[] = await getScores(gameId)
    console.log('Scores fetched:', scores)

    console.log('Updating scores in store...')
    store.dispatch(setScores(scores))
    console.log('Scores updated in store')

    console.log('Payout function completed successfully')
    return Promise.resolve(tx)
  } catch (error) {
    console.error('Error occurred in payout function:', error)
    reportError(error)
    return Promise.reject(error)
  }
}
